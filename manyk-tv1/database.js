...
export { DB_PATH }; // Export for backup system
...
export const Comments = {
   ...
   */
  add(comment) {
    const id = comment.id || `comment_${Date.now()}_${Math.random().toString(36).substr(2, 9)}`;
    const now = new Date().toISOString();
    ...
    return { id, ...comment, createdAt: now };
  },

  /**
   * Content bo'yicha kommentlarni olish
   */
  getByContent(contentId, episodeId = null) {
    const query = episodeId
      ? db.prepare('SELECT * FROM comments WHERE content_id = ? AND episode_id = ? ORDER BY created_at DESC')
      : db.prepare('SELECT * FROM comments WHERE content_id = ? AND episode_id IS NULL ORDER BY created_at DESC');
    
    const rows = episodeId
      ? query.all(contentId, episodeId)
      : query.all(contentId);
  ...
  /**
   * Kommentni o'chirish (admin yoki komment egasi)
   */
  delete(id) {
  ...
  /**
   * Foydalanuvchining barcha kommentlarini olish
   */
  getByUser(userId) {
    const rows = db.prepare('SELECT * FROM comments WHERE user_id = ? ORDER BY created_at DESC').all(userId);
    return rows.map(row => ({
      id: row.id,
      contentId: row.content_id,
      episodeId: row.episode_id,
...
/**
 * Database backuplarini boshqarish.
 * 
 * MUAMMO: Server yangilanganda yoki restart bo'lganda ma'lumotlar
 * yo'qolmasligi uchun avtomatik backup tizimi.
 */
export const Backup = {
  /**
   * Hozirgi database'dan backup yaratadi
   */
  create() {
    try {
      ...
      const backupPath = path.join(BACKUP_DIR, `manyktv-${timestamp}.db`);

      // SQLite BACKUP API ishlatamiz (to'g'ri usul)
      db.exec(`VACUUM INTO '${backupPath.replace(/'/g, "''")}'`);

      console.log(`[Backup] ✅ Backup yaratildi: ${backupPath}`);
      
      // Eski backuplarni tozalash (30 kundan eski)
      this.cleanOldBackups(BACKUP_DIR, 30);
      
      return { success: true, path: backupPath };
    } catch (err) {
      console.error('[Backup] ❌ Backup yaratilmadi:', err);
   ...
   */
  cleanOldBackups(backupDir, daysToKeep = 30) {
    try {
      ...
      for (const file of files) {
        if (!file.startsWith('manyktv-') || !file.endsWith('.db')) continue;
      ...
      if (deletedCount > 0) {
        console.log(`[Backup] 🧹 ${deletedCount} ta eski backup tozalandi`);
      }
    } catch (err) {
      console.error('[Backup] Eski backuplarni tozalashda xato:', err);
    }
  },

  /**
   * Barcha mavjud backuplar ro'yxati
   */
  list() {
    try {
      const BACKUP_DIR = process.env.BACKUP_DIR || path.join(__dirname, 'backups');
      ...
      const files = fs.readdirSync(BACKUP_DIR)
        .filter(f => f.startsWith('manyktv-') && f.endsWith('.db'))
        .map(file => {
      ...
      console.error('[Backup] Ro\'yxatni olishda xato:', err);
      return [];
    }
  },
  ...
  restore(backupPath) {
    try {
      ...
      // Hozirgi DB'ni yopamiz
      db.close();

      // Hozirgi DB'dan backup yaratamiz (xavfsizlik uchun)
      const currentBackupPath = DB_PATH + '.before-restore';
      fs.copyFileSync(DB_PATH, currentBackupPath);

      // Backupdan tiklash
      fs.copyFileSync(backupPath, DB_PATH);

      console.log(`[Backup] ✅ Backup'dan tiklandi: ${backupPath}`);
      console.log(`[Backup] ℹ️  Oldingi versiya saqlandi: ${currentBackupPath}`);

      // Server'ni qayta ishga tushirish kerak
      return { success: true, message: 'Backup tiklandi. Serverni qayta ishga tushiring.' };
    } catch (err) {
      console.error('[Backup] ❌ Backupdan tiklashda xato:', err);
      return { success: false, error: err.message };
    }
  }
};
