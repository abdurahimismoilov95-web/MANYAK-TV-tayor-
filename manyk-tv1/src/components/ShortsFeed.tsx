    ...
    const loadComments = async () => {
      try {
        const response = await fetch(
          `${API_BASE_URL}/comments/${currentDrama.id}?episodeId=${currentEpisode.id}`
        );
        ...
        console.error('[ShortsFeed] Kommentlarni yuklashda xato:', error);
      }
    };
    
    loadComments();
  }, [currentDrama?.id, currentEpisode?.id]);
      ...
      <div className="relative w-full h-full bg-zinc-950 flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute left-4 right-16 bottom-6 z-20 text-left">
          ...
          <div className="flex items-center gap-2 mt-1">
            {/* Episode badge removed - clean look */}
              ...
              <div className="grid grid-cols-4 sm:grid-cols-5 gap-2 overflow-y-auto py-1 flex-1 custom-scrollbar">
                {episodes.map((ep, idx) => {
                  ...
                  return (
                    ...
                    >
                      <span className="text-[10px]">{ep.title || `${ep.episodeNumber || idx + 1}-qism`}</span>
            ...
            <div className="flex-1 overflow-y-auto space-y-2 mb-2 pr-1 custom-scrollbar">
              {comments.length === 0 ? (
                <div className="flex items-center justify-center h-full">
                  <p className="text-xs text-zinc-500">Hozircha kommentlar yo'q. Birinchi bo'ling!</p>
                </div>
              ) : (
                comments.map((c) => (
                  <div key={c.id} className="p-2 rounded-xl bg-zinc-900 border border-zinc-800/80">
                    <div className="flex items-center justify-between gap-1.5 mb-1">
                      <span className="text-[11px] text-red-400 font-bold">{c.username}</span>
