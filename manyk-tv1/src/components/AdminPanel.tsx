...
export const AdminPanel: React.FC<AdminPanelProps> = ({
  ...
  // BROADCAST STATE (for new content announcement)
  ...
  const [broadcastButtonText, setBroadcastButtonText] = useState('Veb appka kirish');
  const [broadcastButtonUrl, setBroadcastButtonUrl] = useState('');
  const [isSendingBroadcast, setIsSendingBroadcast] = useState(false);
  const [isUploadingImage, setIsUploadingImage] = useState(false);

  // CUSTOM DIALOG STATE (for Telegram Web App compatibility)
  interface CustomDialogState {
    ...
    onCancel: () => void;
    onAlternative?: () => void; // For "No, send standard" option
    confirmText?: string;
    cancelText?: string;
  ...
  const allowedTabs = useMemo(() => {
    ...
    if (isSuperAdmin || userPermissions.canBroadcast) {
      list.push({ id: 'broadcast', label: '📢 Xabar Yuborish', icon: Megaphone });
    }
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (data.ok && data.users) {
          ...
          // Server formatida bo'lmasa, localStorage'dan olish
          setUsersList(getStoredUsers());
        }
      } else {
        // Server xato bo'lsa, faqat localStorage'dan ko'rsatamiz
        setUsersList(getStoredUsers());
      }
    } catch (error) {
      console.error('Foydalanuvchilarni yuklashda xato:', error);
      // Xatolik bo'lsa, faqat localStorage'dan ko'rsatamiz
      setUsersList(getStoredUsers());
    }
  ...
  const handleSaveAppointedAdmin = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    const res = await saveAppointedAdmin(
      {
        id: editingAdmin.id,
  ...
  const handleReviewReceipt = async (receiptId: string, decision: 'approved' | 'rejected') => {
    try {
      await reviewReceipt(receiptId, currentUser.id, decision);
      await onRefreshData();
      ...
      if (decision === 'approved') {
        showNotification(
          "✅ To'lov cheki tasdiqlandi! Foydalanuvchiga VIP yoki kontent ochildi!"
        );
      } else {
        showNotification("To'lov cheki rad etildi.");
      }
    } catch (err) {
      showNotification(`❌ Xatolik: ${err instanceof Error ? err.message : 'Chekni ko\'rib chiqishda xatolik'}`);
    }
  };
  ...
  const handleTogglePaymentMethod = async (method: 'checkUpload' | 'clickPayme') => {
    ...
    try {
      await updateStoredSettings(updatedSettings);
      await onRefreshData();
      showNotification(nextVal ? "To'lov usuli faollashtirildi!" : "To'lov usuli o'chirildi!");
    } catch (err) {
      showNotification(`❌ Xatolik: ${err instanceof Error ? err.message : 'Sozlamani yangilashda xatolik'}`);
    }
  };
  ...
  const handleOpenContentEditor = (content?: ContentItem) => {
    ...
    setShouldBroadcast(false); // Reset broadcast checkbox
  ...
  const handleSaveContent = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Agar yangi kontent va broadcast belgilangan bo'lsa, telegram orqali e'lon qilamiz
      const isNewContent = !editingContent.id;
      if (isNewContent && shouldBroadcast) {
        try {
          // Poster URL'ni to'liq manzilga aylantiramiz
          let photoUrl = '';
          if (saved.posterUrl.startsWith('/')) {
            photoUrl = `${window.location.origin}${saved.posterUrl}`;
          } else if (saved.posterUrl.startsWith('http')) {
            photoUrl = saved.posterUrl;
          }
          
          // Agar poster yo'q bo'lsa, banner'dan foydalanamiz
          if (!photoUrl && saved.bannerUrl) {
            if (saved.bannerUrl.startsWith('/')) {
              photoUrl = `${window.location.origin}${saved.bannerUrl}`;
            } else if (saved.bannerUrl.startsWith('http')) {
              photoUrl = saved.bannerUrl;
            }
          }
          
          // Xabar matni - mini drama ham
          const contentType = saved.type === 'movie' ? 'kino' : 
                             saved.type === 'short_drama' ? 'mini drama' : 'serial';
          const text = `🎬 <b>Yangi ${contentType} qo'shildi!</b>\n\n` +
                      `📽 <b>${saved.title}</b>\n` +
                      (saved.originalTitle ? `🌍 ${saved.originalTitle}\n` : '') +
                      `⭐️ ${saved.rating}/10\n` +
                      `📅 ${saved.year}\n` +
                      `🎭 ${saved.genres.join(', ')}\n\n` +
                      `${saved.description.substring(0, 150)}${saved.description.length > 150 ? '...' : ''}\n\n` +
                      `${saved.isPremium ? '💎 Premium kontent' : '✨ Hammaga ochiq'}`;
          
          // Button URL - avtomatik origin ishlatamiz
          const productionUrl = window.location.origin;
          const buttonUrl = `${productionUrl}/?openContent=${saved.id}`;
          ...
          console.error('Broadcast xatosi:', broadcastErr);
          // Broadcast xato bo'lsa ham kontent saqlangan
        }
      }
      
      setIsContentModalOpen(false);
      setShouldBroadcast(false); // Reset
  ...
  const handleConfirmSecondaryAuth = async (e: React.FormEvent) => {
    e.preventDefault();
  ...
  const handleSaveSettings = (e: React.FormEvent) => {
    ...
    // Super Admin uchun ham tasdiqlash dialog
    if (isSuperAdmin) {
      showConfirmDialog(
        'Global Tizim Sozlamalarini Saqlash',
        'Bot tokeni, to\'lov kartalari, 4800 avto-to\'lov va taqiqlangan ro\'yxatlar yangilanadi. Davom etasizmi?',
        async () => {
          try {
            await updateStoredSettings(localSettings);
            ...
            addAuditLog({
              ...
              targetId: 'global',
              targetTitle: 'Global Settings',
              details: 'Global tizim sozlamalari super admin tomonidan yangilandi.',
              secondaryAuthPassed: true,
            });
            setAuditLogs(getStoredAuditLogs());
            
            await onRefreshData();
            showNotification('✅ Sozlamalar muvaffaqiyatli saqlandi!');
          } catch (err) {
            showNotification(`❌ Xatolik: ${err instanceof Error ? err.message : 'Sozlamani saqlashda xatolik'}`);
          }
        }
      );
    } else {
      // Sub-admin uchun parol so'rash
      setPendingAction({
  ...
  // Handler: Broadcast single content to Telegram - SIMPLIFIED VERSION
  const handleBroadcastContent = async (item: ContentItem) => {
    console.log('[Broadcast] Tugma bosildi, kontent:', item.title);
    
    // Avtomatik poster va matn
    const posterUrl = item.posterUrl;
    const text = `🎬 <b>Yangi ${item.type === 'movie' ? 'kino' : item.type === 'short_drama' ? 'mini drama' : 'serial'} qo'shildi!</b>\n\n` +
                 `📽 <b>${item.title}</b>\n\n` +
    ...
    // FAQAT BITTA CONFIRM - Yuborish tasdig'i
    showConfirmDialog(
      `📢 "${item.title}" ni e'lon qilish`,
      `Barcha foydalanuvchilarga xabar yuborilsinmi?\n\n📽 ${item.title}\n📝 ${previewText}...`,
      async () => {
        console.log('[Broadcast] Tasdiqlash: true');
        
        try {
          // Poster URL'ni to'liq manzilga aylantiramiz
          let photoUrl = '';
          if (posterUrl.startsWith('/')) {
            photoUrl = `${window.location.origin}${posterUrl}`;
          ...
          // Agar poster yo'q bo'lsa, banner'dan foydalanamiz
          if (!photoUrl && item.bannerUrl) {
            if (item.bannerUrl.startsWith('/')) {
              photoUrl = `${window.location.origin}${item.bannerUrl}`;
            } else if (item.bannerUrl.startsWith('http')) {
          ...
          // Broadcast yuboramiz
          console.log('[Broadcast] API ga sorov yuborilmoqda...');
          console.log('[Broadcast] photoUrl:', photoUrl);
          console.log('[Broadcast] buttonUrl:', buttonUrl);
          ...
          const data = await res.json();
          console.log('[Broadcast] Server javobi:', data);
          
          if (data.ok) {
            showNotification(`✅ "${item.title}" haqida xabar ${data.totalUsers || 'barcha'} foydalanuvchiga yuborildi!`);
          } else {
            throw new Error(data.error || 'Xabar yuborilmadi');
          }
        } catch (err) {
          showNotification(
            err instanceof Error ? `❌ Xato: ${err.message}` : '❌ Xabar yuborib bo\'lmadi'
          );
        }
      },
      'Ha, yuborish',
  ...
  // USERS MANAGEMENT ACTIONS
  const handleGrantVipToUser = async (userId: string, days: number) => {
    try {
      await grantUserVip(userId, days);
      showNotification(`Telegram ID #${userId} ga ${days} kunlik VIP berildi!`);
      refreshAdminData();
      await onRefreshData();
    } catch (err) {
      showNotification(`❌ Xatolik: ${err instanceof Error ? err.message : 'VIP berishda xatolik'}`);
    }
  };

  const handleRevokeVipFromUser = async (userId: string) => {
    try {
      await revokeUserVip(userId);
      showNotification(`Telegram ID #${userId} ning VIP obunasi bekor qilindi.`);
      refreshAdminData();
      await onRefreshData();
    } catch (err) {
      showNotification(`❌ Xatolik: ${err instanceof Error ? err.message : 'VIP bekor qilishda xatolik'}`);
    }
  };

  const handleToggleUserBanAction = async (userId: string, ban: boolean) => {
    try {
      ...
      showNotification(`❌ Xatolik: ${err instanceof Error ? err.message : 'Ban qilishda xatolik'}`);
    }
  };
  ...
  const handleManualVipGrant = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!manualTelegramId.trim()) return;
    const cleanId = manualTelegramId.trim();
    try {
      await grantUserVip(cleanId, manualVipDays);
  ...
  const handleSaveCatalogAction = async (e: React.FormEvent) => {
    e.preventDefault();
    ...
    try {
      ...
      await saveStoredCatalog(fullCat);
      setIsCatalogModalOpen(false);
      showNotification(`"${fullCat.name}" katalogi ekranga qo'shildi / yangilandi!`);
      refreshAdminData();
      await onRefreshData();
    } catch (err) {
      showNotification(`❌ Xatolik: ${err instanceof Error ? err.message : 'Saqlashda xatolik'}`);
    }
  };
  ...
  const handleDeleteCatalogAction = (id: string, name?: string) => {
    showConfirmDialog(
      'Katalogni o\'chirish',
      `"${name || 'Katalog'}"ni rostdan ham o'chirmoqchimisiz?`,
      () => {
  ...
  const handleAddGenre = async () => {
    ...
    if (!localSettings.availableGenres.includes(clean)) {
      ...
      try {
        await updateStoredSettings(newS);
        setNewGenreInput('');
        await onRefreshData();
        showNotification('✅ Janr qo\'shildi!');
      } catch (err) {
        showNotification(`❌ Xatolik: ${err instanceof Error ? err.message : 'Janr qo\'shishda xatolik'}`);
      }
    }
  };
  ...
  const handleRemoveGenre = async (genre: string) => {
    ...
    try {
      await updateStoredSettings(newS);
      await onRefreshData();
      showNotification('✅ Janr o\'chirildi!');
    } catch (err) {
      showNotification(`❌ Xatolik: ${err instanceof Error ? err.message : 'Janr o\'chirishda xatolik'}`);
    }
  };

  // Custom Dialog Component (for Telegram Web App)
  const CustomDialogModal = () => {
    if (!customDialog?.isOpen) return null;

    const [inputValue, setInputValue] = useState(customDialog.defaultValue || '');

    return (
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
        <div className="bg-zinc-900 rounded-xl max-w-md w-full border border-zinc-800 shadow-2xl animate-fadeIn">
          {/* Header */}
          ...
          <div className="px-6 py-4">
            ...
            {customDialog.type === 'prompt' && (
              <input
                ...
                className="mt-4 w-full px-4 py-3 bg-zinc-800 border border-zinc-700 rounded-lg text-white placeholder-zinc-500 focus:outline-none focus:ring-2 focus:ring-purple-500"
                autoFocus
              />
            )}
          </div>
          ...
          {/* Actions */}
          <div className="px-6 py-4 border-t border-zinc-800 flex gap-3 justify-end flex-wrap">
            {/* Cancel button */}
            <button
            ...
            >
              {customDialog.cancelText || 'Bekor qilish'}
            </button>

            {/* Alternative button (for 3-button dialogs) */}
            {customDialog.onAlternative && (
              <button
                onClick={() => customDialog.onAlternative?.()}
                className="px-4 py-2 rounded-lg bg-blue-600 hover:bg-blue-700 text-white font-medium transition"
              >
                {customDialog.alternativeText || 'Yo\'q'}
              </button>
            )}
    ...
    <div className="fixed inset-0 z-50 bg-black/90 backdrop-blur-lg flex flex-col overflow-hidden select-none">
      ...
      <div className="flex-1 overflow-y-auto p-4 max-w-6xl w-full mx-auto">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0 flex flex-col justify-between">
                    ...
                    <div className="flex items-center gap-1.5 pt-2 border-t border-zinc-800/80 justify-end">
                      {/* Telegram'da e'lon qilish */}
                      <button
                        onClick={() => handleBroadcastContent(item)}
                        title="Telegram'da e'lon qilish"
                        className="p-1.5 rounded bg-zinc-800 hover:bg-blue-500/10 text-zinc-400 hover:text-blue-400 border border-zinc-700/50 hover:border-blue-500/30 transition-colors flex items-center justify-center"
                      >
                  ...
                  <div className="flex items-center gap-2 mt-4 pt-3 border-t border-zinc-800 justify-end">
                    ...
                    <button
                      onClick={() => {
                        ...
                        if (isSuperAdmin) {
                          showConfirmDialog(
                            `"${plan.name}" tarifini o'chirish`,
                            `"${plan.name}" obuna tarifi butunlay o'chiriladi. Davom etasizmi?`,
                            async () => {
                              try {
                                console.log('[AdminPanel] Deleting plan:', plan.id);
                                await deleteStoredPlan(plan.id);
                                console.log('[AdminPanel] Plan deleted, refreshing data...');
                                ...
                                addAuditLog({
                                  ...
                                  details: `"${plan.name}" obuna tarifi super admin tomonidan o'chirildi.`,
                                  secondaryAuthPassed: true,
                                });
                                setAuditLogs(getStoredAuditLogs());
                                ...
                                showNotification(`❌ Xatolik: Tarifni o'chirib bo'lmadi. Qayta urinib ko'ring.`);
                              }
                            }
                          );
                        } else {
                  ...
                  <button
                    onClick={() => {
                      showConfirmDialog(
                        ...
                        `"${pr.code}" promokodini rostdan ham o'chirmoqchimisiz?`,
                        async () => {
                          try {
                            await deleteStoredPromoCode(pr.id);
                            await onRefreshData();
                            showNotification('Promokod muvaffaqiyatli o\'chirildi!');
                          } catch (err) {
                            console.error('[Delete Promo] Error:', err);
                            showNotification('❌ Xatolik: Promokodni o\'chirib bo\'lmadi.');
                          }
                        }
                      );
                    }}
        ...
        {activeTab === 'catalogs' && (
          <div className="space-y-3">
            <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-2">
              <div>
                <h3 className="text-sm font-bold text-white flex items-center gap-1.5">
                  ...
                  <span>Ekran Kataloglari</span>
                </h3>
                <p className="text-[10px] text-zinc-400">
                  Bosh sahifada chiqadigan kataloglar
                </p>
              </div>
              ...
              <button
                ...
                className="flex items-center gap-1 px-3 py-1.5 rounded-lg bg-red-600 hover:bg-red-500 font-bold text-[10px] text-white transition self-start sm:self-auto"
              >
            ...
            {/* Explanatory Banner */}
            <div className="p-2.5 rounded-xl bg-zinc-900/90 border border-zinc-800 text-[10px] text-zinc-300 flex items-center gap-2">
              <div className="w-6 h-6 rounded-lg bg-red-950 border border-red-800 flex items-center justify-center text-red-400 flex-shrink-0">
                <Tv className="w-3.5 h-3.5" />
              </div>
              <p>
                Har bir katalog bosh sahifada alohida bo'lim. <strong className="text-white">Mini Drama</strong> - <strong className="text-red-400">9:16 vertikal</strong> format.
              </p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-2.5">
              {catalogsList.map((cat) => {
                ...
                return (
                  <div
                    key={cat.id}
                    className="p-3 rounded-xl bg-zinc-900/90 border border-zinc-800 flex flex-col justify-between gap-2.5 hover:border-zinc-700 transition"
                  >
                    <div>
                      <div className="flex items-center justify-between mb-1.5">
                        <div className="flex items-center gap-1.5">
                          ...
                          {cat.badge && (
                            <span className="text-[8px] font-extrabold px-1.5 py-0.5 rounded bg-red-950 text-red-400 border border-red-800 uppercase">
                              {cat.badge}
                            </span>
                          )}
                        </div>
                        ...
                        <button
                          ...
                          className={`text-[9px] font-bold px-2 py-0.5 rounded-lg transition border ${
                            cat.isVisible !== false
                              ? 'bg-emerald-950/60 border-emerald-700 text-emerald-300'
                              : 'bg-zinc-800 border-zinc-700 text-zinc-400'
                          }`}
                        >
                          {cat.isVisible !== false ? 'Faol' : 'Yashirin'}
                        </button>
                      </div>
                      ...
                      <div className="flex flex-wrap items-center gap-1.5 mb-1.5">
                        {cat.format === 'vertical_9_16' ? (
                          <span className="flex items-center gap-1 text-[9px] font-black text-pink-400 bg-pink-950/40 border border-pink-800/60 px-2 py-0.5 rounded-lg">
                            <Smartphone className="w-3 h-3 text-pink-400" />
                            <span>9:16 Vertikal</span>
                          </span>
                        ) : (
                          <span className="flex items-center gap-1 text-[9px] font-semibold text-zinc-300 bg-zinc-800/80 border border-zinc-700 px-2 py-0.5 rounded-lg">
                        ...
                        <span className="text-[9px] text-zinc-400 font-mono">
                          Filmlar: <strong className="text-white">{assignedCount}</strong>
                        </span>
                      </div>

                      {cat.description && (
                        <p className="text-[10px] text-zinc-400 line-clamp-2">{cat.description}</p>
                      )}
                    </div>

                    <div className="flex items-center justify-end gap-1 pt-1.5 border-t border-zinc-800">
                      <button
                        type="button"
                        onClick={() => handleOpenCatalogModal(cat)}
                        title="Tahrirlash"
                        className="p-1 rounded-lg bg-zinc-800 hover:bg-zinc-700 text-zinc-400 hover:text-emerald-400 transition flex items-center justify-center"
                      >
                        <Edit className="w-3.5 h-3.5" />
                      </button>
                      ...
                      <button
                        ...
                        title="O'chirish"
                        className="p-1 rounded-lg bg-zinc-800 hover:bg-red-900/60 text-zinc-400 hover:text-red-400 transition flex items-center justify-center"
                      >
                        <Trash2 className="w-3.5 h-3.5" />
                      </button>
                    </div>
        ...
        {/* 7.5. BROADCAST / XABAR YUBORISH TAB */}
        {activeTab === 'broadcast' && (
          <div className="space-y-4 max-w-2xl">
            <div>
              <h3 className="text-base font-bold text-white flex items-center gap-2">
              ...
              <p className="text-xs text-zinc-400 mt-1">
                Telegram bot orqali barcha foydalanuvchilarga bildirishnoma yuboring
              </p>
            </div>
              ...
              <div className="flex items-start gap-2">
                ...
                <div className="text-xs text-amber-300 leading-relaxed">
                  ...
                  <ul className="space-y-1 list-disc list-inside">
                    <li>Xabar barcha ro'yxatdan o'tgan foydalanuvchilarga yuboriladi</li>
                    <li>Telegram bot sozlamalari to'g'ri bo'lishi kerak</li>
                    <li>Yuborish jarayoni bir necha daqiqa davom etishi mumkin</li>
                  </ul>
                </div>
              </div>
            </div>
            ...
            <form
              onSubmit={async (e) => {
                ...
                if (!broadcastTitle.trim() || !broadcastMessage.trim()) {
                  showNotification('❌ Sarlavha va xabar matnini kiriting!');
                  return;
                }
                ...
                try {
                  ...
                  const formattedText = `<b>${broadcastTitle}</b>\n\n${broadcastMessage}`;
                  ...
                  if (broadcastImage.trim()) {
                    payload.photoUrl = broadcastImage.trim();
                  }

                  // Add button if provided
                  if (broadcastButtonText.trim() && broadcastButtonUrl.trim()) {
                    payload.buttonText = broadcastButtonText.trim();
                    payload.buttonUrl = broadcastButtonUrl.trim();
                  }
                  ...
                  if (data.ok) {
                    showNotification(`✅ Xabar ${data.totalUsers || 'barcha'} foydalanuvchiga yuborilmoqda!`);
                    // Clear form
                    setBroadcastTitle('');
                    setBroadcastMessage('');
                    setBroadcastImage('');
                    setBroadcastButtonText('Veb appka kirish');
                    setBroadcastButtonUrl('');
                  } else {
                    throw new Error(data.error || 'Xabar yuborilmadi');
                  }
                } catch (err) {
                  showNotification(`❌ Xatolik: ${err instanceof Error ? err.message : 'Xabar yuborishda xatolik'}`);
                } finally {
            ...
            >
              ...
              <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-2xl space-y-3">
                ...
                <input
                  ...
                  placeholder="Masalan: Yangi seriallar qo'shildi!"
                  className="w-full bg-zinc-950 border border-zinc-700 rounded-xl px-3 py-2.5 text-sm text-white placeholder-zinc-500 outline-none focus:border-blue-500 transition"
                  required
                />
              </div>
              ...
              <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-2xl space-y-3">
                ...
                <textarea
                  ...
                  placeholder="Foydalanuvchilarga yuboriladigan asosiy xabar matni..."
                  rows={6}
                  className="w-full bg-zinc-950 border border-zinc-700 rounded-xl px-3 py-2.5 text-sm text-white placeholder-zinc-500 outline-none focus:border-blue-500 transition resize-none"
                  required
                />
                <p className="text-xs text-zinc-500">
                  Belgilar: {broadcastMessage.length} / 1000
                </p>
              </div>
              ...
              <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-2xl space-y-3">
                ...
                <div className="space-y-2">
                  <label className="block">
                    <input
                      ...
                      onChange={async (e) => {
                        ...
                        if (file.size > 5 * 1024 * 1024) {
                          showNotification('❌ Fayl hajmi 5MB dan oshmasligi kerak!');
                          return;
                        }
                        ...
                        try {
                          ...
                          if (data.ok && data.fullUrl) {
                            setBroadcastImage(data.fullUrl);
                            showNotification('✅ Rasm yuklandi!');
                          } else {
                            throw new Error(data.error || 'Yuklashda xatolik');
                          }
                        } catch (err) {
                          showNotification(`❌ ${err instanceof Error ? err.message : 'Yuklashda xatolik'}`);
                        } finally {
                    ...
                    <div className="flex items-center gap-2">
                      <span
                        className={`flex-1 px-4 py-2.5 rounded-xl border text-sm text-center cursor-pointer transition ${
                          isUploadingImage
                            ? 'bg-zinc-800 border-zinc-700 text-zinc-500 cursor-wait'
                            : 'bg-zinc-950 border-zinc-700 text-blue-400 hover:border-blue-500 hover:bg-zinc-900'
                        }`}
                      >
                        {isUploadingImage ? (
                      ...
                      {broadcastImage && (
                        <button
                          ...
                          className="px-3 py-2.5 rounded-xl bg-red-950 border border-red-800 text-red-400 hover:bg-red-900 transition text-sm"
                        >
                          <X className="w-4 h-4" />
                        </button>
                      )}
                ...
                <div className="relative">
                  ...
                  <div className="relative flex justify-center text-xs">
                    <span className="px-2 bg-zinc-900 text-zinc-500">yoki</span>
                  </div>
                </div>
                ...
                {broadcastImage && (
                  <div className="mt-2 p-2 bg-zinc-950 border border-zinc-800 rounded-lg">
                    <p className="text-xs text-zinc-400 mb-2">Rasm ko'rinishi:</p>
                    <img
                      src={broadcastImage}
                      alt="Preview"
              ...
              <div className="p-4 bg-zinc-900/90 border border-zinc-800 rounded-2xl space-y-3">
                ...
                <div className="grid grid-cols-1 gap-3">
                  <div>
                    ...
                    <input
                      ...
                      placeholder="Veb appka kirish"
                      className="w-full bg-zinc-950 border border-zinc-700 rounded-xl px-3 py-2 text-sm text-white placeholder-zinc-500 outline-none focus:border-blue-500 transition"
                    />
                  </div>
                  <div>
                    <label className="block text-xs text-zinc-400 mb-1">
                ...
                <p className="text-xs text-zinc-500">
                  💡 Foydalanuvchilar xabar ostidagi tugmani bosib veb appga o'tishlari mumkin
                </p>
              </div>

              {/* Preview Box */}
              {(broadcastTitle || broadcastMessage) && (
                <div className="p-4 bg-zinc-900/90 border border-blue-800/40 rounded-2xl space-y-3">
                  <div className="flex items-center gap-2">
                  ...
                  <div className="p-3 bg-zinc-950 border border-zinc-800 rounded-xl space-y-2">
                    ...
                    {broadcastMessage && (
                      <p className="text-xs text-zinc-300 whitespace-pre-wrap">{broadcastMessage}</p>
                    )}
                  </div>
                </div>
              )}
              ...
              <button
                ...
                className="w-full py-3 px-4 rounded-xl bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-500 hover:to-blue-600 disabled:opacity-50 disabled:cursor-not-allowed font-bold text-sm text-white shadow-lg shadow-blue-600/30 transition flex items-center justify-center gap-2"
              >
                {isSendingBroadcast ? (
                  <>
                  ...
                  <>
                    ...
                    <span>Barcha Foydalanuvchilarga Yuborish</span>
                  </>
                )}
              </button>
            </form>
      ...
      {isContentModalOpen && editingContent && (
        <div className="fixed inset-0 z-60 bg-black/90 backdrop-blur-md flex items-center justify-center p-3 sm:p-4 overflow-y-auto">
          <div className="relative w-full max-w-2xl bg-[#121216] border border-zinc-800 rounded-xl p-4 shadow-2xl my-auto max-h-[90vh] overflow-y-auto">
            <button
              onClick={() => {
                setIsContentModalOpen(false);
                setShouldBroadcast(false);
              }}
              className="absolute top-3 right-3 p-1 rounded-lg bg-zinc-800/80 text-zinc-400 hover:text-white hover:bg-zinc-700 transition"
            >
              <X className="w-4 h-4" />
            </button>
            ...
            <div className="flex items-center gap-2 mb-3 pb-3 border-b border-zinc-800">
              ...
              <h3 className="text-base font-black text-white">
                {editingContent.id ? 'Kontentni Tahrirlash' : 'Yangi Kontent'}
              </h3>
            </div>
            ...
            <form onSubmit={handleSaveContent} className="space-y-3">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="block text-[10px] font-bold text-zinc-400 mb-1 uppercase tracking-wide">
                    Nomi:
                  </label>
                  <input
                    type="text"
                    ...
                    className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-2.5 py-1.5 text-xs text-white outline-none focus:border-red-500 transition"
                  />
                </div>
                ...
                >
                  ...
                  {catalogsList.map((cat) => (
                    <option key={cat.id} value={cat.id}>
                      {cat.name} {cat.format === 'vertical_9_16' ? '(9:16)' : '(16:9)'}
                    </option>
                  ))}
                </select>
              </div>
              ...
              <div className="p-3 bg-zinc-900/90 rounded-xl border border-zinc-800 space-y-2.5">
                <div className="text-[10px] font-black text-red-400 flex items-center gap-1.5 uppercase tracking-wider">
                  <CreditCard className="w-3.5 h-3.5" />
                  <span>Monetizatsiya Sozlamalari</span>
                </div>

                <div className="grid grid-cols-2 gap-2">
                  <label className="flex items-center gap-2 cursor-pointer text-xs font-semibold text-zinc-200 bg-zinc-950/80 p-2 rounded-lg border border-zinc-800 hover:border-zinc-700 transition">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span className="text-[11px]">VIP'ga kiradi</span>
                  </label>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer text-xs font-semibold text-zinc-200 bg-zinc-950/80 p-2 rounded-lg border border-zinc-800 hover:border-zinc-700 transition">
                    ...
                    />
                    <span className="text-[11px]">Alohida sotuvda</span>
                  </label>
                  ...
                  <label className="flex items-center gap-2 cursor-pointer text-xs font-semibold text-zinc-200 bg-zinc-950/80 p-2 rounded-lg border border-zinc-800 hover:border-zinc-700 transition">
                    <input
                      ...
                      className="w-3.5 h-3.5 accent-amber-500 rounded"
                    />
                    <span className="text-[11px]">⭐ #1 Premyera</span>
                  </label>
                ...
                <div>
                  ...
                  <input
                    ...
                    placeholder="15000"
                    className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-2.5 py-1.5 text-xs font-bold text-red-400 outline-none focus:border-red-500 transition"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  className="w-full bg-zinc-900 border border-zinc-700 rounded-lg px-2.5 py-1.5 text-xs text-white outline-none focus:border-red-500 transition resize-none"
              ...
              {(editingContent.type === 'series' || editingContent.type === 'anime_series' || editingContent.type === 'short_drama') && (
                <div className="p-4 bg-zinc-900 rounded-xl border border-zinc-800 space-y-3">
                  ...
                  <div className="grid grid-cols-4 gap-2 max-h-96 overflow-y-auto pr-1 custom-scrollbar">
                    {modalEpisodes.map((ep, idx) => (
                      <div key={ep.id} className="relative flex flex-col rounded-lg bg-zinc-950/80 border border-zinc-800/80 hover:border-zinc-700/80 transition-colors group">
                        
                        {/* Thumbnail Area - Vertikal poster */}
                        <div className="relative w-full aspect-[2/3] rounded-t-lg overflow-hidden bg-zinc-900">
                          <img 
                            src={editingContent?.posterUrl || ''} 
                            alt="Ep Thumbnail" 
                          ...
                          <div className="absolute bottom-1 left-1 right-1 px-1.5 py-1 text-center">
                            <span className="text-white font-black text-[10px] drop-shadow-[0_1px_3px_rgba(0,0,0,0.8)]">
                        ...
                        <div className="flex flex-col p-1.5 flex-1">
                          <input
                            ...
                            className="bg-transparent border-b border-transparent hover:border-zinc-700 focus:border-red-500 text-[10px] font-bold text-white outline-none w-full truncate pb-0.5 transition-colors placeholder:text-zinc-600 text-center"
                            placeholder={`${ep.episodeNumber}-qism`}
                          />
                          ...
                          <div className="mt-auto pt-1.5 flex flex-col gap-1">
                             ...
                             <button
                               ...
                               className={`w-full px-1.5 py-0.5 text-[8px] font-extrabold rounded text-center transition-colors ${
                             ...
                             <div className="flex items-center gap-1">
                               <label className="flex-1 flex items-center justify-center h-6 bg-zinc-800 hover:bg-zinc-700 text-zinc-300 rounded cursor-pointer transition-colors">
                               ...
                               <button
                                 ...
                                 className="flex items-center justify-center w-6 h-6 bg-zinc-800 hover:bg-red-500/20 text-zinc-500 hover:text-red-400 rounded transition-colors"
                               >
              ...
              {/* Telegram'da e'lon qilish checkbox (faqat yangi kontent uchun) */}
              {!editingContent?.id && (
                  ...
                  <label className="flex items-start gap-3 cursor-pointer">
                    ...
                    <div>
                      ...
                      <div className="text-[11px] text-zinc-400 mt-0.5">
                        Saqlangandan keyin barcha obunachilarga bot orqali avtomatik xabar yuboriladi
                      </div>
                    </div>
                  </label>
                </div>
              ...
              <div className="flex gap-2 pt-2">
                ...
                {editingPlan.id && (
                  <button
                    ...
                    onClick={async () => {
                      try {
                        await deleteStoredPlan(editingPlan.id as string);
                        setIsPlanModalOpen(false);
                        onRefreshData();
                        showNotification('Tarif muvaffaqiyatli o\'chirildi!');
                      } catch (err) {
                        console.error('[Delete Plan Modal] Error:', err);
                        showNotification('❌ Xatolik: Tarifni o\'chirib bo\'lmadi.');
                      }
                    }}
          ...
          <div className="relative max-w-md w-full bg-[#121216] border border-zinc-800 rounded-2xl p-5">
            ...
            <form
              onSubmit={async (e) => {
                ...
                try {
                  await saveStoredPromoCode(editingPromo as PromoCode);
                  setIsPromoModalOpen(false);
                  await onRefreshData();
                  showNotification('✅ Promokod muvaffaqiyatli saqlandi!');
                } catch (err) {
                  showNotification(`❌ Xatolik: ${err instanceof Error ? err.message : 'Saqlashda xatolik'}`);
                }
              }}
      ...
      {/* SECONDARY VERIFICATION MODAL FOR SENSITIVE ADMIN ACTIONS - FAQAT SUB-ADMINLAR UCHUN */}
