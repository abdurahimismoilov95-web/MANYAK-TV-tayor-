...
import { getStoredHistory, getStoredFavorites, toggleFavorite } from '../services/storage';
import { getAuthHeaders } from '../services/authToken';
import { showConfirm, showAlert } from '../utils/customDialogs';
...
export const HistoryView: React.FC<HistoryViewProps> = ({
  ...
  const handleClear = () => {
    // Custom dialog orqali tasdiqlash
    showConfirm(
      "Tarixni tozalash",
      "Ko'rish tarixini butunlay o'chirmoqchimisiz? Bu amalni bekor qilib bo'lmaydi.",
      () => {
        // Darhol localStorage'ni tozalaymiz
        try {
          localStorage.removeItem('manyak_tv_history_v1');
          
          // Storage event dispatch qilamiz
          window.dispatchEvent(new CustomEvent('manyak_storage_update'));
          
          showAlert('Muvaffaqiyatli', 'Ko\'rish tarixi muvaffaqiyatli tozalandi!');
          onRefresh();
        } catch (error) {
          console.error('[HistoryView] localStorage tozalashda xato:', error);
          showAlert('Xatolik', 'Tarixni tozalashda xatolik yuz berdi');
        }
      }
    );
  };
      ...
      <div className="flex justify-end">
        {activeTab === 'history' && historyList.length > 0 && (
          <button
            onClick={handleClear}
            disabled={isClearing}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-zinc-900 hover:bg-red-950/80 text-zinc-400 hover:text-red-400 text-xs font-semibold border border-zinc-800 transition disabled:opacity-50 disabled:cursor-not-allowed"
          >
