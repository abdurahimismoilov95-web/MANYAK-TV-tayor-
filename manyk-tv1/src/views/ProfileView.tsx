...
export const ProfileView: React.FC<ProfileViewProps> = ({
  ...
  const handleClearCache = () => {
    showConfirm(
      "Keshni tozalash",
      "Keshni va ko'rishlar tarixini tozalashni xohlaysizmi? Bu qurilmangizdagi joyni bo'shatadi.",
      () => {
        ...
        showAlert("Muvaffaqiyatli", "Kesh va ko'rishlar tarixi muvaffaqiyatli tozalandi!");
      ...
      <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-sm p-4">
        <div className="bg-zinc-900 rounded-xl max-w-md w-full border border-zinc-800 shadow-2xl animate-fadeIn">
          ...
          <div className="px-6 py-4 border-t border-zinc-800 flex gap-3 justify-end">
            {customDialog.type === 'confirm' && customDialog.onCancel && (
              <button
  ...
  return (
    <div className="p-4 pb-28 space-y-5 max-w-2xl mx-auto">
      ...
      <div className="p-4 rounded-3xl bg-gradient-to-b from-zinc-900 via-zinc-900/90 to-[#121216] border border-zinc-800/80 shadow-xl flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 w-full min-w-0">
          ...
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2">
              ...
              {isAdmin && (
                <span className="text-[8px] font-extrabold bg-red-950 text-red-400 border border-red-800 px-1 py-0.5 rounded uppercase">
                  ADMIN
                </span>
              )}
            </div>
            ...
            <div className="mt-1">
              {user.isPhoneVerified ? (
                <span className="inline-flex items-center gap-1 text-[8px] font-semibold text-emerald-400 bg-emerald-950/60 border border-emerald-800/60 px-1.5 py-0.5 rounded">
                  <ShieldCheck className="w-2.5 h-2.5" />
                  <span className="truncate">Tel: {user.phone || '+'}</span>
                </span>
              ) : (
                <button
                  onClick={onOpenVerifyPhone}
                  className="inline-flex items-center gap-1 text-[8px] font-bold text-amber-400 bg-amber-950/60 border border-amber-800/60 px-1.5 py-0.5 rounded hover:border-amber-500 transition active:scale-95"
                >
        ...
        <button
          onClick={onOpenVip}
          className={`w-10 h-10 flex-shrink-0 rounded-xl flex flex-col items-center justify-center transition border ${
            user.isVip
        ...
        >
          ...
          <span className="text-[7px] font-black mt-0.5 uppercase tracking-wider">
            {user.isVip ? 'VIP' : 'Olish'}
          </span>
        </button>
      </div>

      {/* 2. VIP Subscription Status & Countdown Monitor */}
      <div className="p-3 rounded-2xl bg-gradient-to-br from-[#15151c] to-[#121216] border border-zinc-800 relative overflow-hidden">
        {/* Glow effect */}
        ...
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5">
            ...
            <h4 className="text-xs font-black text-white">VIP Obuna</h4>
          </div>
        ...
        {user.isVip ? (
          <div className="space-y-2">
            <div className="flex items-center justify-between p-2 rounded-xl bg-zinc-900/80 border border-zinc-800/80">
              <div className="flex items-center gap-1.5">
                ...
                <div>
                   <div className="text-[9px] text-zinc-400">Qolgan:</div>
                   <div className="text-xs font-black text-white">{vipRemainingDays} kun</div>
                </div>
              </div>

              <div className="text-right">
                <div className="text-[9px] text-zinc-400">Tugaydi:</div>
            ...
            {isExpiringSoon && (
              <div className="p-2 rounded-xl bg-gradient-to-r from-amber-950/80 to-red-950/60 border border-amber-500/60 flex items-center justify-between gap-2">
                <div className="flex items-center gap-2">
                  ...
                  <div className="text-[10px] font-black text-amber-300">
                    {vipRemainingDays} kundan tugaydi!
                  </div>
                </div>
                <button
                  onClick={onOpenVip}
                  className="px-2.5 py-1 rounded-lg bg-gradient-to-r from-amber-500 to-orange-500 hover:from-amber-400 hover:to-orange-400 text-zinc-950 font-black text-[9px] whitespace-nowrap transition active:scale-95 flex items-center gap-1"
                >
            ...
            <p className="text-[10px] text-zinc-400">
              Barcha kino va seriallar ochiq!
            </p>

            <button
              onClick={onOpenVip}
              className={`w-full py-2 px-3 rounded-xl font-black text-[10px] transition flex items-center justify-center gap-1.5 ${
                isExpiringSoon
                  ? 'bg-gradient-to-r from-amber-500 via-orange-500 to-red-600 hover:from-amber-400 hover:to-orange-400 text-white'
                  : 'bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-zinc-950'
              }`}
            >
              <Zap className="w-3.5 h-3.5 fill-current" />
              <span>
                {isExpiringSoon ? "Uzaytirish (39,000)" : 'Uzaytirish'}
              </span>
            </button>
          </div>
        ) : (
          <div className="space-y-2">
            <p className="text-[10px] text-zinc-400">
              VIP obuna bilan barcha kontentni 1 oylik cheksiz tomosha qiling.
            </p>
            <button
              onClick={onOpenVip}
              className="w-full py-2 px-3 rounded-xl bg-red-600 hover:bg-red-500 text-white font-bold text-[10px] transition flex items-center justify-center gap-1.5"
            >
              <Sparkles className="w-3.5 h-3.5 text-amber-300" />
              <span>VIP Faollashtirish (39,000)</span>
            </button>
          </div>
        )}
      </div>
      ...
      <div className="p-3 rounded-2xl bg-zinc-900/90 border border-zinc-800 space-y-2">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-1.5">
            ...
            <h4 className="text-xs font-black text-white">Kunlik Kirish</h4>
          </div>
          {user.dailyCheckIn?.streak ? (
            <span className="inline-flex items-center gap-1 text-[9px] font-black text-amber-400 bg-amber-950/80 border border-amber-800/80 px-2 py-0.5 rounded-full">
              <Flame className="w-2.5 h-2.5 fill-amber-400" />
              <span>{user.dailyCheckIn.streak} kun</span>
            </span>
          ) : (
            <span className="text-[9px] text-zinc-500 font-medium">Yangi</span>
          )}
        </div>
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="p-2.5 rounded-xl bg-zinc-950/80 border border-zinc-800/80">
            ...
            <div className="text-lg font-black text-amber-400">
              {user.accessTokens || 0} <span className="text-[9px] text-zinc-400 font-bold">ta</span>
            </div>
            <p className="text-[8px] text-zinc-500 mt-0.5">
              Seriallarda har 10 qismga 1 ta qism ochish
            </p>
          </div>
          ...
          <div className="p-2.5 rounded-xl bg-zinc-950/80 border border-zinc-800/80">
            ...
            <div className="text-lg font-black text-emerald-400">
              {(user.bonusBalance || 0).toLocaleString()} <span className="text-[9px] text-zinc-400 font-bold">UZS</span>
            </div>
            <p className="text-[8px] text-zinc-500 mt-0.5">
              Chegirma sifatida
            </p>
      ...
      {/* 3. My Purchased Dramas */}
      <div className="p-3 rounded-2xl bg-zinc-900/80 border border-zinc-800">
        <div className="flex items-center justify-between mb-2">
          <div className="flex items-center gap-1.5">
            <Film className="w-3.5 h-3.5 text-red-500" />
            <h4 className="text-xs font-bold text-white">Xarid Qilinganlar</h4>
          </div>
        ...
        {purchasedItems.length === 0 ? (
          <p className="text-[10px] text-zinc-500">
            Alohida xarid qilingan dramalar yo'q.
          </p>
        ) : (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {purchasedItems.map((item) => (
              <div
                ...
                className="p-2 rounded-xl bg-zinc-950 border border-zinc-800 hover:border-zinc-700 cursor-pointer transition flex items-center gap-2"
              >
                <img
                  src={item.posterUrl}
                  alt={item.title}
      ...
      {/* 3.5. Individual Movie & Series Store */}
