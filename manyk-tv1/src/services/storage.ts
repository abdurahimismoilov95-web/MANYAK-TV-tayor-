...
// Update system settings - SERVER API orqali
export async function updateStoredSettings(updates: Partial<SystemSettings>): Promise<SystemSettings> {
  console.log('[updateStoredSettings] Starting...', Object.keys(updates));
  try {
    const authHeaders = await getAuthHeaders();
    console.log('[updateStoredSettings] Auth headers:', authHeaders);
    ...
    console.log('[updateStoredSettings] Response status:', response.status);

    if (!response.ok) {
      const error = await response.json();
      console.error('[updateStoredSettings] Server error:', error);
      throw new Error(error.error || 'Sozlamalarni yangilashda xatolik');
    }
    ...
    localStorage.setItem('manyak_tv_settings_v1', JSON.stringify(merged));
    ...
    console.log('[updateStoredSettings] ✅ Success!');
    return merged;
  } catch (err) {
    console.error('[updateStoredSettings] ❌ Error:', err);
    throw err;
  }
}
...
export async function saveStoredCatalog(catalog: CatalogCategory): Promise<void> {
  const settings = getStoredSettings();
  ...
  await updateStoredSettings({ catalogs });
}
...
// Save plan - SERVER API orqali
export async function saveStoredPlan(plan: SubscriptionPlan): Promise<void> {
  console.log('[saveStoredPlan] Starting...', plan.id);
  try {
    const authHeaders = await getAuthHeaders();
    const isNewPlan = !getStoredPlans().some(p => p.id === plan.id);
    
    console.log('[saveStoredPlan] Auth headers:', authHeaders);
    console.log('[saveStoredPlan] Is new plan:', isNewPlan);
    
    const response = await fetch(
      isNewPlan ? `${API_BASE_URL}/plans` : `${API_BASE_URL}/plans/${plan.id}`,
      {
    ...
    console.log('[saveStoredPlan] Response status:', response.status);

    if (!response.ok) {
      const error = await response.json();
      console.error('[saveStoredPlan] Server error:', error);
      throw new Error(error.error || 'Tarifni saqlashda xatolik');
    }
    ...
    localStorage.setItem('manyak_tv_plans_v1', JSON.stringify(all));
    console.log('[saveStoredPlan] ✅ Success!');
  } catch (err) {
    console.error('[saveStoredPlan] ❌ Error:', err);
    throw err;
  }
}

export async function deleteStoredPlan(id: string): Promise<void> {
  console.log('[deleteStoredPlan] Starting deletion for plan ID:', id);
  
  // Server'ga o'chirish so'rovini yuborish
  try {
    const authHeaders = await getAuthHeaders();
    console.log('[deleteStoredPlan] Got auth headers, sending DELETE request...');
    ...
    console.log('[deleteStoredPlan] Response status:', response.status);
    
    if (!response.ok) {
      const errorText = await response.text();
      console.error('[deleteStoredPlan] Server error response:', errorText);
      throw new Error(`Server error: ${response.status} - ${errorText}`);
    }
    
    const result = await response.json();
    console.log('[deleteStoredPlan] ✅ Plan successfully deleted from server:', result);
    
    // Client-side'dan ham o'chiramiz (to'g'ridan-to'g'ri localStorage)
    const all = getStoredPlans().filter((p) => p.id !== id);
    localStorage.setItem('manyak_tv_plans_v1', JSON.stringify(all));
    console.log('[deleteStoredPlan] ✅ Plan removed from localStorage');
    
  } catch (err) {
    console.error('[deleteStoredPlan] ❌ Error:', err);
    throw err; // Re-throw to show error notification
  }
}
...
// Save promo code - SERVER API orqali
export async function saveStoredPromoCode(promo: PromoCode): Promise<void> {
  try {
    const authHeaders = await getAuthHeaders();
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'Promokodni saqlashda xatolik');
    }
    ...
    localStorage.setItem('manyak_tv_promo_codes_v1', JSON.stringify(all));
  } catch (err) {
    console.error('[saveStoredPromoCode] Error:', err);
    throw err;
  }
}

export async function deleteStoredPromoCode(id: string): Promise<void> {
  console.log('[deleteStoredPromoCode] Starting deletion for promo code ID:', id);
  
  try {
    const authHeaders = await getAuthHeaders();
    console.log('[deleteStoredPromoCode] Got auth headers, sending DELETE request...');
    ...
    console.log('[deleteStoredPromoCode] Response status:', response.status);
    
    if (!response.ok) {
      const errorText = await response.text();
      console.error('[deleteStoredPromoCode] Server error response:', errorText);
      throw new Error(`Server error: ${response.status} - ${errorText}`);
    }
    
    const result = await response.json();
    console.log('[deleteStoredPromoCode] ✅ Promo code successfully deleted from server:', result);
    
    // Client-side'dan ham o'chiramiz
    const all = getStoredPromoCodes().filter((p) => p.id !== id);
    localStorage.setItem('manyak_tv_promo_codes_v1', JSON.stringify(all));
    console.log('[deleteStoredPromoCode] ✅ Promo code removed from localStorage');
    
  } catch (err) {
    console.error('[deleteStoredPromoCode] ❌ Error:', err);
    throw err;
  }
}
...
// Grant VIP to any user by Telegram ID - SERVER API orqali
export async function grantUserVip(userId: string, days: number): Promise<UserProfile | null> {
  try {
    const authHeaders = await getAuthHeaders();
    const response = await fetch(`${API_BASE_URL}/users/${userId}/vip`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        ...authHeaders,
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'VIP berishda xatolik');
    }
    ...
    // LocalStorage'ni yangilash
    const all = getStoredUsers();
    const userIndex = all.findIndex((u) => u.id === userId);
    if (userIndex >= 0) {
      all[userIndex] = data.user;
    } else {
      all.unshift(data.user);
    }
    setItem(KEYS.USERS, all);
    
    // Agar joriy foydalanuvchi bo'lsa, uni ham yangilash
    const current = getItem<UserProfile | null>(KEYS.CURRENT_USER, null);
    if (current && current.id === userId) {
      saveStoredCurrentUser({ ...current, ...data.user });
    }
    
    return data.user;
  } catch (err) {
    console.error('[grantUserVip] Error:', err);
    throw err;
  }
}

// Revoke VIP from user - SERVER API orqali
export async function revokeUserVip(userId: string): Promise<void> {
  try {
    const authHeaders = await getAuthHeaders();
    const response = await fetch(`${API_BASE_URL}/users/${userId}/vip`, {
      method: 'DELETE',
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'VIP bekor qilishda xatolik');
    }
    ...
    console.error('[revokeUserVip] Error:', err);
    throw err;
  }
}

// Block / Unblock Telegram ID - SERVER API orqali
export async function toggleUserBan(userId: string, ban: boolean, reason?: string): Promise<void> {
  try {
    const authHeaders = await getAuthHeaders();
    const url = `${API_BASE_URL}/users/${userId}/ban`;
    ...
    const response = await fetch(url, {
      ...
      body: ban ? JSON.stringify({ reason: reason || 'Admin tomonidan bloklandi' }) : undefined,
    });

    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'Ban qilishda xatolik');
    }

    // LocalStorage settings'ni yangilash
    const settings = getStoredSettings();
    ...
    await updateStoredSettings({ bannedUserIds: bannedIds });
...
export async function saveAppointedAdmin(
...
): Promise<{ success: boolean; message: string; admin?: AppointedAdmin }> {
  ...
  try {
    ...
    return {
      success: false,
      message: `Xatolik: ${err instanceof Error ? err.message : 'Saqlashda xatolik'}`,
    };
  }
}
...
// Review receipt (approve/reject) - SERVER API orqali
export async function reviewReceipt(
  ...
  try {
    const authHeaders = await getAuthHeaders();
    const response = await fetch(`${API_BASE_URL}/receipts/${receiptId}/review`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
        ...authHeaders,
    ...
    if (!response.ok) {
      const error = await response.json();
      throw new Error(error.error || 'Chekni ko\'rib chiqishda xatolik');
    }
    ...
    if (rcpt) {
      ...
      localStorage.setItem('manyak_tv_receipts_v1', JSON.stringify(all));
    }
